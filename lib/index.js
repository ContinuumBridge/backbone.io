    ...
    Object.keys(backends).forEach(function(backend) {
        io.of(backend).on('connection', function(socket) {
            ...
            socket.on('listen', function(channel, callback) {
                var ch = socket.handshake.channel || channel;
                if (ch) {
                    socket.set('channel', ch, function() {
                    socket.join(ch);
                        callback(options);
                    });
                } else {
            ...
            socket.on('sync', function(req, callback) {
                ...
                sync.handle(backends[backend], req, function(err, result) {
                    ...
                    if (!err && req.method !== 'read') {
                        ...
                        if (channel) {
                            socket.server.to(channel).emit('synced', req.method, result);
                        } else {
                            socket.server.to(socket.id).emit('synced', req.method, result);
                        }
                    }
                });
            });
            ...
            var events = { 'created': 'create', 'updated': 'update', 'deleted': 'delete', 'reset': 'reset' };
            Object.keys(events).forEach(function(event) {
                var listener = function(model, backendchannel) {
                    socket.get('channel', function(err, channel) {
                        if (!backendchannel){
                            socket.emit('synced', events[event], model);
                        }
                        else if(backendchannel === channel){
                            socket.emit('synced', events[event], model);
                        }
                    });
                };
